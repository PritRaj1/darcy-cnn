...
function (m::FNOBlock)(x, ps, st)
    x_w, st_c = m.conv(x, ps.conv, st.conv)                    # W*v (skip)
    x_k, st_s = m.spect_conv(x, ps.spect_conv, st.spect_conv)  # K*v (spectral)
    x_m, st_m = m.mlp(x_k, ps.mlp, st.mlp)                     # MLP(K*v)
    return m.activation.(x_m .+ x_w), (spect_conv = st_s, mlp = st_m, conv = st_c)
end
