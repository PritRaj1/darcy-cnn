ENV["TF_GPU_ALLOCATOR"] = get(ENV, "TF_GPU_ALLOCATOR", "cuda_malloc_async")

include("src/WavKANConv.jl")

using .WavKANConv
using Random
using Lux
using Lux: Training
using Reactant
using Optimisers
using ProgressBars
using JLD2
using MLDataDevices: reactant_device, cpu_device

model_name = length(ARGS) >= 1 ? ARGS[1] : "CNN"
rep = length(ARGS) >= 2 ? parse(Int, ARGS[2]) : 1
cfg = load_config(model_name)
dev = reactant_device()

train_loader, test_loader = get_darcy_loader(cfg.batch_size; dev = dev)
rng = Random.default_rng()

log_dir = joinpath("logs", model_name)
model_dir = joinpath(log_dir, "trained_models")
mkpath(model_dir)

Random.seed!(rep)
model = create_model(cfg)
ps, st = Lux.setup(rng, model)
ps = ps |> Lux.f32 |> dev
st = st |> Lux.f32 |> dev

train_state = Training.TrainState(model, ps, st, Optimisers.Adam(cfg.learning_rate, (0.9f0, 0.999f0), 1.0f-8))

log_file = joinpath(log_dir, "repetition_$rep.csv")
open(log_file, "w") do f
    write(f, "Epoch,Time (s),Train Loss,Test Loss,BIC\n")
end

loss_fn(y_pred, y) = loss_fcn(y_pred, y; p = cfg.p)
eval_fwd = WavKANConv.compile_eval(model, ps, st, test_loader, loss_fn)
start_time = time()

n_params = Lux.parameterlength(model)
n_samples = size(first(train_loader)[1], 4)

for epoch in ProgressBar(1:(cfg.num_epochs))
    global train_state, tl, vl = train_epoch(
        train_state, train_loader, test_loader,
        loss_fn, model, eval_fwd, epoch, cfg,
    )
    log_csv(epoch, tl, vl, BIC(n_params, n_samples, vl), time() - start_time, log_file)
    GC.gc()
end

ps_cpu = train_state.parameters |> cpu_device()
jldsave(joinpath(model_dir, "model_$rep.jld2"); ps = ps_cpu, model_name = model_name)
